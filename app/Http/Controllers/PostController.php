...
class PostController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $this->post->image = "data:image/".$request->image->extension().";base64,".base64_encode (file_get_contents($request->image)); //画像ファイルの保存形式、毎回サーバーに取りに行く
        $this->post->save();
        ...
        //     ['category_id' => 2]
        // ]
        $this->post->categoryPost()->createMany($category_posts);
    ...
    public function edit($id){
        ...
        $selected_categories = [];
        foreach($post_a->categoryPost as $category_post){
    ...
    public function update(Request $request, $id){
        ...
        $post_a->categoryPost()->delete();

        $category_posts = [];
        foreach($request->categories as $category_id){
            $category_posts []= ['category_id' => $category_id];
        }
        $post_a->categoryPost()->createMany($category_posts);
